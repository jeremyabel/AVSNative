...
std::vector<PresetAsset> Texer::CollectAssets() const
{
    if (m_raw.empty())
        return {};
...
void Texer::ApplyAsset(const std::string& /*key*/, const std::string& name,
...
void Texer::BuildFromRaw(const std::vector<uint8_t>& raw)
{
    ResetAnimation();
