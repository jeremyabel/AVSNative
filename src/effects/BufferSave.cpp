...
void BufferSave::Init(bgfx::RendererType::Enum /*Renderer*/)
{
    {
    ...
    {
        bgfx::ShaderHandle VS = bgfx::createShader(bgfx::copy(vs_fullscreen_spv, sizeof(vs_fullscreen_spv)));
        bgfx::ShaderHandle FS = bgfx::createShader(bgfx::copy(fs_buffersave_blend_spv, sizeof(fs_buffersave_blend_spv)));
...
{
    const float Params[4] = { (float)Cfg.BlendMode, Cfg.BlendAmt, 0.0f, 0.0f };
...
{
    FBOSlot& Scratch  = Context.FboManager->GetScratch(Cfg.Slot);
    const uint16_t W  = Context.FboManager->GetWidth();
    const uint16_t H  = Context.FboManager->GetHeight();

    if (Cfg.BlendMode == 0)
    {
        // Replace: blit input → scratch (aux view executes after main, but both read from
        // unmodified InputTexture so order doesn't matter)
    ...
    else
    {
        ...
        //   ViewId   → OutputFBO : blend  (intermediate)
...
void BufferSave::Render(const RenderContext& Context)
{
    ...
    else
    {
        ...
        doSave = saveFirst ? !AltPhase : AltPhase;
        AltPhase = !AltPhase;
    }
