...
static void DrawInterferencesUI(Effect* base)
{
    ...
    ImGui::TextUnformatted("Num Points");
    ImGui::SetNextItemWidth(-1.0f);
    ...
    SliderF("Alpha (On Beat)",     "##alpha2",       &fx->Alpha2,       1.0f,  255.0f);
    SliderF("Distance (On Beat)",  "##distance2",    &fx->Distance2,    1.0f,   64.0f);
    SliderF("Rot Speed (On Beat)", "##rotationinc2", &fx->RotationInc2, -32.0f, 32.0f);
    ...
    ImGui::Checkbox("RGB Separation", &fx->RGB);
